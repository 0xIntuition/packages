import type { SchemaOrgTypeSpec } from '../../types.js';

export const schemaOrgMedicalTrial = {
	id: 'schema:MedicalTrial',
	name: 'MedicalTrial',
	label: 'MedicalTrial',
	comment:
		'A medical trial is a type of medical study that uses a scientific process to compare the safety and efficacy of medical therapies or medical procedures. In general, medical trials are controlled and subjects are allocated at random to the different treatment and/or control groups.',
	subClassOf: ['MedicalStudy', 'MedicalEntity', 'Thing'],
	properties: [
		{
			id: 'schema:trialDesign',
			name: 'trialDesign',
			label: 'trialDesign',
			comment: 'Specifics about the trial design (enumerated).',
			rangeIncludes: ['MedicalTrialDesign'],
		},
	],
} as const satisfies SchemaOrgTypeSpec;

export const spec = schemaOrgMedicalTrial;
export const MedicalTrial = schemaOrgMedicalTrial;

export default schemaOrgMedicalTrial;
