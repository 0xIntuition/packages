...
| ID | Task | Files | Size |
|---|---|---|---|
| A1 | Add fields to `PredicateSpec` (flat): `polarity`, `objectKind`, `temporalNature`, `claimType`, `isSymmetric` (exists), `isTransitive` (exists), `inverse: PredicateKey`, `specializes: PredicateKey`, `contradicts: PredicateKey[]` | `src/types.ts` | S |
| A2 | `definePredicateRecord`: defaults + **derivation/validation** — `isSymmetric ⟹ inverse=self` (reject conflicting `inverse`); inverse pairs mirror algebraic flags; `contradicts` symmetric on both sides; typed-key refs resolve to known keys (build fails otherwise) | `src/record.ts` | M |
...
| A6 | New generator `scripts/generate-predicate-seed.mjs` → emits `dist/predicate-seed.json` (and optional `.sql`): one row per predicate with `{ id: PREDICATE_IDS[key], slug: key, label, description, isSymmetric, isTransitive, inverse→id, specializes→id, contradicts→[id], polarity, objectKind, temporalNature, claimType, marketPattern }` — references resolved to atom ids | `scripts/`, `package.json` build | M |
| A7 | Tests: derivation/validation unit tests; seed snapshot test | `src/*.test.ts` | S |
...
| B2 | Add predicate columns | `specializes_predicate_id text`, `polarity text`, `object_kind text`, `market_pattern text`, `claim_type text` (or keep `claim_type`/`temporal_nature` in `metadata` JSONB per the column-vs-JSONB heuristic) |
| B3 | Add `temporal_nature` | JSONB-first (`metadata->>'temporalNature'`); promote later if filtered |
| B4 | Contradictions table | `CREATE TABLE kg.predicate_contradictions (predicate_id text, contradicts_id text, PRIMARY KEY (predicate_id, contradicts_id))` — store both directions (seed writes symmetric pairs) |
...
| C3 | Contradictions seed | Populate `kg.predicate_contradictions` from the artifact (both directions). |
...
| D6 | (Phase 4) Materialize hot paths | inferred reverse edges + closure table, written `inferred=true` + provenance; **excluded from aggregates that count the origin** | partial indexes |
